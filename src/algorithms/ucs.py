...
def uniform_cost_search(board_info, start_state) -> SearchResult:
    start_time = time.perf_counter()

    pq = []
    heap_id = 0
    heapq.heappush(pq, (0, heap_id, start_state))
    ...
    while pq:
        current_cost, _, current_state = heapq.heappop(pq)
        iterations += 1
        ...
        if is_goal(board_info, current_state):
            ...
            return make_result(True, moves, states, current_cost, iterations, time_ms)
        ...
        for neighbor in get_valid_neighbors(board_info, current_state):
            ...
            if next_state not in best_cost or new_cost < best_cost[next_state]:
                ...
                heap_id += 1
                heapq.heappush(pq, (new_cost, heap_id, next_state))
